"""
Dataset Generator for FastText Skills Model (Project Matcher)
...
def generate_corpus():
    ...
    # --- 4. Project descriptions (project-matcher focused) ---
    project_templates = [
        ("web application", ["frontend", "backend", "database"]),
        ("mobile app", ["mobile", "backend", "database"]),
        ("e-commerce platform", ["frontend", "backend", "database", "cloud"]),
        ("social media platform", ["frontend", "backend", "database", "cloud"]),
        ("data pipeline", ["data", "database", "cloud"]),
        ("machine learning project", ["ai_ml", "data", "programming_languages"]),
        ("cloud infrastructure", ["cloud", "devops", "linux"]),
        ("api service", ["api", "backend", "database"]),
        ("dashboard", ["frontend", "backend", "database", "data"]),
        ("content management system", ["frontend", "backend", "database"]),
        ("real time chat application", ["frontend", "backend", "database"]),
        ("video streaming platform", ["frontend", "backend", "cloud"]),
        ("payment gateway", ["backend", "database", "security"]),
        ("inventory management system", ["frontend", "backend", "database"]),
        ("healthcare platform", ["frontend", "backend", "database", "security"]),
        ("education platform", ["frontend", "backend", "database"]),
        ("fintech application", ["backend", "database", "security", "cloud"]),
        ("iot platform", ["backend", "database", "cloud", "devops"]),
        ("devops automation tool", ["devops", "cloud", "linux"]),
        ("testing framework", ["testing", "backend"]),
    ]

    for project_name, categories in project_templates:
        for _ in range(50):
            skills = []
            ...
            sentences.append(f"project {project_name} requires team with {' '.join(skills)}")
            sentences.append(f"team for {project_name} project needs skills in {' '.join(skills)}")
            sentences.append(f"building {project_name} uses {' '.join(skills)}")
            sentences.append(f"hiring team for {project_name} looking for {' '.join(skills)}")

    # --- 5. Synonym / alias sentences ---
    aliases = [
    ...
    # --- 7. Contextual similarity sentences ---
    similar_groups = [
...
def main():
    print("Generating tech skills training corpus (Project Matcher)...")
    random.seed(43)  # Different seed from team recommender
